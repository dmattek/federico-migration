...
myFreadNuc = function(fileIn) {
  ...
  # list of typically removed columns
  s.col.loc = c(
    'Metadata_C',
  ...
  s.col.rem.loc = intersect(s.head, s.col.loc)

  if (length(s.col.rem.loc) > 0)
    dt.loc[, (s.col.rem.loc) := NULL]
...
myFreadImg = function(fileIn) {
  ...
  # remove unnecessary columns if they exist
  # list of typically removed columns
  s.col.loc = c('Metadata_FileLocation', 
          'Group_Index', 
          'Group_Number')
  s.col.rem.loc = intersect(names(dt.loc), s.col.loc)
